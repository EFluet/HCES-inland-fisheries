...
l_abs <- ggplot(protein.df) +
  ...
  annotate("text", label='World median\n GDP/cap', 
           x=median(wdi_gdp$NY.GDP.PCAP.CD), 
           y=35, 
           angle= 90, 
           color='grey', vjust=1.1, fontface="italic", size=2.5) +
  
  annotate("text", label='survey country \nmedian GDP/cap', 
           x=median(protein.df$NY.GDP.PCAP.CD), 
           y=35, 
           angle= 90, 
           color='grey', vjust=1.1, fontface="italic", size=2.5) +
  ...
  annotate("text", label='FAO', 
           x=2100, y=65, 
           color='black', size=4.5) +
  
  
  geom_step(aes(x= NY.GDP.PCAP.CD, y= cum.fao.pers.equiv/10^6)) +
  geom_step(aes(x= NY.GDP.PCAP.CD, y= cum.rev.fao.pers.equiv/10^6), color='red') +
  scale_x_log10() +
  ylab('Cumulative \n person equivalent (x10^6)') +
  xlab('GDP per capita') 
