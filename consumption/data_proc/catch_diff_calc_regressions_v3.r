...
f <- hces_surv_asprcfao ~  
            ...
            Catch_flag +
            AG.LND.TOTL.K2  + GLWD + 
            SP.RUR.TOTL.ZS #+ sum_catch # + SP.RUR.TOTL
...
# plot(seq_len(nrow(bestmodel$Subsets)) - 1, bestBIC$Subsets[,"BIC"], type="b", xlab = "Number of Predictors", ylab = "BIC")
...
ggplot() +
  ...
  geom_point(data=surv_data,
             aes(x=hces_surv_asprcfao, y=psmulti, size=sum_catch), alpha=0.3) +   
  #, color= hces_surv_asprcfao_cut
  
  # add labels
  geom_text_repel(data= subset(surv_data, (hces_surv_asprcfao > 210 | psmulti > 250)),
