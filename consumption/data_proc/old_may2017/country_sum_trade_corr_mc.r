...
for (j in seq(nrow(consump_nat_agg_df))) {
  ...
  # get Monte Carlo vector of imports 
  if (length(cntry_sum_imp_fwae_assumed_mc[[temp_cntry_code]])==TRUE){
  ...
  # add uncertainty of matched FWAE from import and export to matched FWAE uncertainty from consumption  
...
rm(aq, cons, exp, imp, j, temp_cntry_code, exp_ur, imp_ur, tr_corr_ur, temp_outdf,
   uncert_prodfm_ur, tr_corr_consump, cons_pretrcorr)
