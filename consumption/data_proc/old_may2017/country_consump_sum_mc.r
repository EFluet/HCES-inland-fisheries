...
consump_df <- read.csv('../output/consumption/hh_survey_consump_sep_prods_ref_fwaes.csv', stringsAsFactors=FALSE)
...
for (j in seq(uniq_cntry)) {
  ...
  for (i in seq(nrow(temp_consump_df))) {
    ...
    # if the processing is unknown, overright the 
    # if (is.na(temp_consump_df[i,'product'])) {
    #   prod_fwae_fac_mc <- mcstoc(runif, 
    #                              min=  temp_consump_df[i,'min_conv_fac'], 
    #                              max=  temp_consump_df[i,'max_conv_fac'])
    #   }
...
write.csv(consump_nat_agg_df_mcperc, '../output/consumption/hh_survey_consump_nat_aggprod_refu_fw_mc.csv')
