  ...
  for (i in seq(nrow(temp_tr_fwae))) {
    ...
    if (i == 1){
      ...
      if(cur_tradeflow == 'Export' & is.na(cur_prod_type)){
        sum_exp_fwae_assumed_mc <- prod_fwae_mc}
      
      if(cur_tradeflow == 'Export' & !is.na(cur_prod_type)){
        sum_exp_fwae_matched_mc <- prod_fwae_mc}
      
      if(cur_tradeflow=='Import' & is.na(cur_prod_type)) {
        sum_imp_fwae_assumed_mc <- prod_fwae_mc}
      
      if(cur_tradeflow=='Import' & !is.na(cur_prod_type)) {
        sum_imp_fwae_matched_mc <- prod_fwae_mc}
    }
    

    if (i > 1){
    
      if(cur_tradeflow == 'Export' & is.na(cur_prod_type) & exists('sum_exp_fwae_assumed_mc')){
        sum_exp_fwae_assumed_mc <- sum_exp_fwae_assumed_mc + prod_fwae_mc}
      
      if(cur_tradeflow == 'Export' & !is.na(cur_prod_type) & exists('sum_exp_fwae_matched_mc')){
        sum_exp_fwae_matched_mc <- sum_exp_fwae_matched_mc + prod_fwae_mc}
      
      if(cur_tradeflow=='Import' & is.na(cur_prod_type) & exists('sum_imp_fwae_assumed_mc')) {
        sum_imp_fwae_assumed_mc <- sum_imp_fwae_assumed_mc + prod_fwae_mc}
      
      if(cur_tradeflow=='Import' & !is.na(cur_prod_type) & exists('sum_imp_fwae_matched_mc')) {
