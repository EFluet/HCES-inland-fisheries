...
prod_src_m_cnt_perctry <-  consump_df %>%
  group_by(country_code, prod_src_fm) %>%
  summarise(src_fm_count = n()) %>%
  filter(prod_src_fm == 'M' & src_fm_count == sum(src_fm_count))
# ungroup() %>%
...
consump_df <- consump_df %>% 
  ...
  filter(!country_code  %in%  prod_src_m_cnt_perctry$country_code)
...
# remove products listed as marine (no more, because used for updating F:M ratio)
consump_df <- consump_df %>% 
...
for (j in seq(uniq_cntry)) {
  ...
  temp_unique_gdp <- unique(temp_consump_df[,'meanNY.GDP.PCAP.CD'])
  
  
  ### Make prediction over countries, for use in survey calculation
  temp_boot <-  sampled_models %>% 
    rowwise %>% 
    do(data.frame(z_hat = predict(.$model,
                                  list(x = temp_unique_clpa, y = temp_unique_gdp))))
  # remove negative simulations
  temp_boot <- temp_boot %>% filter(z_hat > 0, 
                                    z_hat < 1)
  
  # count the number of negatives simulations
  nb_neg_boot <- 10000 - nrow(temp_boot)
  
  # resample missing probs from positive distribution 
  temp_boot<- c(temp_boot$z_hat, (sample(x = temp_boot$z_hat, size = nb_neg_boot, replace = T)))
  #, ngrid = 90, new.xpts = NULL, weights = NULL)
  #tot_fm_boot_preds<- samples(temp_boot, name = c("fitted"))
  tot_fm_boot_preds <- temp_boot
  ...
  # Weight of Marine products?  Filtered out?
  ...
  # if weight of unknown is not equal to total weight
  if (W_u != W_t){
    W_k <- (W_m + W_f)
    R_k <- W_f / (W_m + W_f)
    R_t <- tot_fm_boot_preds
    
    R_u <- (W_t * R_t - W_k * R_k) / W_u   # ratio of unknown
    
    replace(R_u, R_u < 0, 0)  # because negative ratio is not possible, give floor value of zero
    
    unres_boot_preds <- R_u
  }else{unres_boot_preds <- tot_fm_boot_preds}
  
  
  # After
  temp_consump_df <- temp_consump_df %>% filter(prod_src_fm != 'M')
  ...
  # append mcnode into mc object with the summed vector for each country
...
rm(x, y, z, W_u, W_t, W_m, W_k, W_f, R_k, R_t, R_u)

# plot hist and save
hist(cntry_sum_prod_fwae_mc)
dev.copy(png, '../output/figures/myplot.png')
dev.off()
...
consump_nat_agg_df_mcperc <-  consump_nat_agg_df %>% 
  ...
  # # country label with the parentheses  
  ...
  mutate(country_label= ifelse(year_end==0, 
                               paste(country.x,', ', year_start, sep=''),
                               paste(country.x,', ', year_start, '-', str_sub(year_end,-2,-1), sep=''))) %>%
...
#consump_nat_agg_df_mcperc[,c('tr_corr_consump','p025','p975')]

surv_means <- consump_nat_agg_df_mcperc %>% 
  dplyr::select(country_code, country_label, year_start, year_end, datatype, mean) %>%
  filter(datatype %in% c('sum_catch', 'tr_corr_consump')) %>%
  #unite(comb, p025, mean, p975, datatype) %>%
  spread(datatype, mean) 

consump_nat_agg_df_mcperc_diff <-  consump_nat_agg_df_mcperc %>% 
  dplyr::select(country_code, datatype, p025, p975) %>%
  filter(datatype %in% c('tr_corr_consump')) %>%
  left_join(., surv_means, by='country_code') %>%
  mutate(tr_corr_consump = as.numeric(tr_corr_consump),
         sum_catch = as.numeric(sum_catch),
         p025= as.numeric(p025),
         p975 = as.numeric(p975),
         mean_dif = (tr_corr_consump-sum_catch),
         p025_dif = (p025-sum_catch),
         p975_dif = (p975-sum_catch),
                     mean_dif_perc = (tr_corr_consump-sum_catch)/sum_catch *100,
                     p025_dif_perc = (tr_corr_consump-sum_catch)/sum_catch *100,
                     p975_dif_perc = (tr_corr_consump-sum_catch)/sum_catch *100)


# Write the table to csv file ------------------------------------------------
write.csv(consump_nat_agg_df_mcperc, '../output/consumption/hh_survey_consump_nat_aggprod_refuse_fw_mc_gamfm.csv')
write.csv(consump_nat_agg_df_mcperc_diff, '../output/consumption/hh_survey_consump_nat_aggprod_refuse_fw_mc_gamfm_diff.csv')
