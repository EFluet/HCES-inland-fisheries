...
#glob_preds <- glob_preds %>% filter(Country == "Cote d'Ivoire")
...
glob_preds$psmulti_pred_avg <- c(psmulti$averages)
...
glob_preds_forplot <- glob_preds %>%
              ...
              #filter(psmulti_pred_avg < 1250) %>%
              mutate(pred_catch = psmulti_pred_avg/100 * sum_catch)  %>%
...
ggplot(glob_preds_forplot) +
  geom_histogram(aes(x=psmulti_pred_avg), stat='bin', position='stack')
...
### add in the missing countries ---------------

nat_agg_df2 <- nat_agg_df %>%
              left_join(., glob_preds_forplot, by='country_code') %>%
              mutate(fao_catch_M = sum_catch.x / 10^6) %>%
              select(country_code, fao_catch_M, pred_catch) %>%
              mutate(agg_pred_fao_catch = ifelse(is.na(pred_catch), fao_catch_M, pred_catch))
...
sum(glob_preds_forplot$sum_catch)
...
sum(nat_agg_df2$fao_catch_M, na.rm = TRUE)
sum(nat_agg_df2$pred_catch, na.rm = TRUE)
sum(nat_agg_df2$agg_pred_fao_catch, na.rm = TRUE)
