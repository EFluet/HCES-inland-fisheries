...
t <- ggplot() +
  ...
  geom_point(data=glob_sum_surv, aes(x=2008, y=tr_corr_consump), size=1.1, color='blue') +

  geom_errorbar(data=glob_sum_surv, aes(x=2008, ymin=p025, ymax=p975), 
                color='blue', size=0.5, width=0) +

  # *** add mid line to rectangle --------------------------------------------------
  geom_segment(data = glob_sum_surv, color='blue', size=0.5,
  ...
  geom_text_repel(data = glob_sum_surv,
                  aes(x=min_year_start, y=tr_corr_consump, label= "Survey-estimated catch in\n42 countries"),
                  color='blue',
                  segment.color='blue',
                  size = 2.5,
                  nudge_x = -5.5,
...
ggsave('../output/figures/fig4_global_flag_timeline_v5.pdf', t, 
       width=87, height=64, dpi=800, units="mm")  #, type = "cairo-png")
dev.off()
