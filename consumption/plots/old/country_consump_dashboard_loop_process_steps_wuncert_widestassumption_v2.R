...
for (i in consump_nat_agg_df$country) {
  ...
  cols <- c('country', 
            ...
            "import_miltons","export_miltons",
            "aquacult_miltons","catch_miltons",
            'cons_miltonspyr_fm_min_fwaes',
            "cons_miltonspyr_fm_max_fwaes",
  ...
  temp_consump_nat_agg_df <-  consump_nat_agg_df %>%
                              select(one_of(cols)) %>%
                              gather(col_names, mid_value, cons_miltonspyr_fm_mean_fwaes:catch_miltons) %>%
    
    # subset data to current country in loop, and select columns
    filter(country == i) %>% # & col_names %in% cols) %>%
    
    mutate(min_value = ifelse(col_names=="cons_miltonspyr_fm_mean_fwaes",cons_miltonspyr_fm_min_fwaes, NA),
           max_value = ifelse(col_names=="cons_miltonspyr_fm_mean_fwaes",cons_miltonspyr_fm_max_fwaes, NA),
           min_value = ifelse(col_names=="cons_miltonspyr_fm_mean_fwaes_notr",cons_miltonspyr_fm_min_fwaes_notr, min_value),
           max_value = ifelse(col_names=="cons_miltonspyr_fm_mean_fwaes_notr",cons_miltonspyr_fm_max_fwaes_notr, max_value)) %>%
   

    # rename columns names for nicer plot labels
    mutate(col_names = ifelse(col_names=='export_miltons','Freshwater Export (FishStatJ)',col_names),
           col_names = ifelse(col_names=='import_miltons','Freshwater Import (FishStatJ)',col_names),
           col_names = ifelse(col_names=='catch_miltons','Inland Catch (FishStatJ)',col_names),
           col_names = ifelse(col_names=='cons_miltonspyr_fm_mean_fwaes','HH Survey consumption (FWEA)',col_names),
           col_names = ifelse(col_names=='cons_miltonspyr_fm_mean_fwaes_notr','HH Survey consumpion (FWEA, trade corr.)',col_names),
    ...
    select(country, cols_wrap, mid_value, min_value, max_value) %>%
  ...
  outfile <- paste("../output/figures/consump_corr_steps_larger_uncert/",i,"_fw_ctch_consumpsurvey_corr_steps.png",sep="")
