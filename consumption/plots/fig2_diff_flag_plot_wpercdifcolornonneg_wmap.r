...
# ~~~ inset baplpot subset countries for inset ----------
surv_data_forinsetbar<- surv_data %>% filter(mean_dif > -0.04 & mean_dif < 0.05)
...
dif_barplot <- ggplot(surv_data, 
  ...
  theme(plot.margin=unit(c(3, 1, -5, -2), "mm"),
  ...
  xlab('') + ylab('Difference of survey and reported catch (×1000 tonnes)') +
...
mean_dif_histogram  <- 
  ...
  xlab('') + ylab('Nb. countries')+ 
  theme(legend.position = "none",
        axis.text.x=element_blank(),
        plot.margin=unit(c(5, 1, -6, 0), "mm"),
...
source('./consumption/maps/fig1_map_hhsurvey_percdiff_lattrim.r')
...
t5 = arrangeGrob(g5, ncol=1)



fig2_final_plot <- plot_grid(g3, g4, align = "v", nrow = 2, rel_heights = c(1/8, 7/8))
xx <- arrangeGrob(fig2_final_plot)
fig2_final_plot_v2 <- plot_grid(xx, g5, align = "v", nrow = 2, rel_heights = c(16/20, 4/20))
...
### save plot ------------------------------------------------------------------
ggsave("../Output/Figures/fig2_barplot_percdiff_wmap_v3.pdf", fig2_final_plot_v2,
       dpi=1000, width=87, height=165, units='mm')# , type = "cairo-png")
