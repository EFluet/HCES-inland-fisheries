...
#~~ count nb of countries reporting inl catch ------------
ca_inl_sel_cnt <- ca_inl_sel %>%
                  filter(num_val > 0)


length(unique(ca_inl_sel_cnt$CountryCountry)) - 42 - 2
...
t <- ggplot() +
  ...
  annotate("text", label='Other 189 countries', x=1988, y=5.2, angle = 35, color='white', size=2.9) +
  annotate("text", label='1997-2014', x=2006, y=11.8, color='blue', hjust=0.5, fontface="italic", size=2.5) +
  ...
  #                 fill='blue', color=NA, alpha=0.20, inherit.aes = FALSE) +

  geom_point(data=glob_sum_surv, aes(x=2008, y=tr_corr_consump), size=1, color='blue') +

  geom_errorbar(data=glob_sum_surv, aes(x=2008, ymin=p025, ymax=p975), 
                color='blue', size=0.4, width=0) +

  # *** add mid line to rectangle --------------------------------------------------
  geom_segment(data = glob_sum_surv, color='blue', size=0.4,
  ...
  geom_text_repel(data = glob_sum_surv,
                  aes(x=min_year_start, y=tr_corr_consump, label= "Survey-estimated catch"),
                  color='blue',
                  segment.color='blue',
                  size = 2.5,
                  nudge_x = -5.5,
  ...
  # *** add global catch line --------------------------------------------------
  geom_line(data=glob_ca_sum, aes(x= year, y=catch_fao), color='black', size=0.3) +
  ...
  coord_cartesian(expand=0, xlim= c(1950, 2014), ylim= c(0, 12.5)) +
  
  ylab('Inland wild fish catch (million tonnes)') + xlab('Year') +
...
### *** save figure to file --------------------------------------------------------
ggsave('../output/figures/fig4_global_flag_timeline_v5_simplified_w2014label.pdf', t, 
