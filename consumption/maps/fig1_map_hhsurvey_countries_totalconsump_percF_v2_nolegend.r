...
consump_prod_fwaes <- read.csv('../output/consumption/hh_survey_consump_sep_prods_refuse_fwaes_manual_prod_match.csv', stringsAsFactors = F)
...
consump_prod_fwaes<- consump_prod_fwaes %>%
                     ...
                     summarize(prod_weight = sum(consump_million.tons.yr)) %>%
...
my_breaks = c(0, 100, 200, 300, 500, 1000)
...
inland_map_w_inset = map_plot_inlandf + annotation_custom(grob = g2, xmin= -16 *10^6, 
                                                         xmax= -7 *10^6, 
                                                         ymin= -7.5 *10^6, 
                                                         ymax=  1.5 *10^6)
...
mostfwae_map_w_inset = map_plot_mostfwae + annotation_custom(grob = g3, xmin= -16 *10^6, 
                                                          xmax= -7 *10^6, 
                                                          ymin= -7.5 *10^6, 
                                                          ymax=  1.5 *10^6)
...
hces_prod_map_w_inset = map_plot_hces_prod + annotation_custom(grob = g4, xmin= -16 *10^6, 
                                                           xmax= -7 *10^6, 
                                                           ymin= -7.5 *10^6, 
                                                           ymax=  1.5 *10^6)
...
t1 = arrangeGrob(g1,ncol=1, left = textGrob("A", y = 0.95, vjust=1, gp=gpar(fontsize=12)))
t2 = arrangeGrob(g2,ncol=1, left = textGrob("B", y = 0.95, vjust=1, gp=gpar(fontsize=12)))
t3 = arrangeGrob(g3,ncol=1, left = textGrob("C", y = 0.95, vjust=1, gp=gpar(fontsize=12)))
t4 = arrangeGrob(g4,ncol=1, left = textGrob("D", y = 0.95, vjust=1, gp=gpar(fontsize=12)))


final = arrangeGrob(t1,t2,t3,t4, ncol=1)
final_plot <- grid.arrange(final)



### Save figure to file --------------------------------------------------------
ggsave('../output/figures/maps/map_hhsurvey_consump_winset_v2_wrefuse_4plots.png',  final_plot,
       width=90, height=185, dpi=800, units="mm", type = "cairo-png")
dev.off()
