...
t4 = arrangeGrob(g4,ncol=1, left = textGrob("A", y = 0.9, x= 1.1, vjust=1, gp=gpar(fontsize=12)))
t5 = arrangeGrob(g5,ncol=1, left = textGrob("B", y = 0.9, x= 1.1, vjust=1, gp=gpar(fontsize=12)))


final = arrangeGrob(t4,t5, ncol=1)
final_plot <- grid.arrange(final)



### Save figure to file --------------------------------------------------------
ggsave('../output/figures/fig1_map_consump_prod_diff_1col_noconsump.pdf',  final_plot,
       width=87, height=95, dpi=800, units="mm")  #, type = "cairo-png")
dev.off()
